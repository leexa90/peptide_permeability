...
neg = []
for line in open('./raw/peptide_bioactive.csv','r'): #signalling from all 
    if '\n' in line:
        line = line[:-1]
    neg += [line,]
...
neg1 = neg_seq
neg = pd.read_csv('./raw/peptide_PDB.csv') #pDB
...
neg = pd.read_csv('./raw/Mammal_signal_peptide.csv') #
...
neg = []
for line in open('./raw/peptide_CPP2.csv','r'): #CPP2
    if '\n' in line:
        line = line[:-1]
    if '>' not in line:
        neg += [line,]
...
data.to_csv('./processed/bioactive_PDB_cpp.csv',index=0)
