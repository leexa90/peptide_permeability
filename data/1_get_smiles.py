...
data  = pd.read_csv('./raw/dataset_stapled.csv')
...
def save_smile(i,counter ):
    ...
    try:
        ...
        if 'temp_%s.sdf'%(counter) not in os.listdir('./structures'):
            open('./structures/temp_%s.sdf'%(counter),'w').write(Chem.MolToMolBlock(mol))
        return smile_str
    except :
...
data['list'] = data['sequence'].apply(process_string)
print (data['sequence'].apply(process_string))
counter = 0
smile = []
for i in data['sequence']:
    ...
    counter += 1
data['SMILES'] = smile
data.to_csv('./interim/stapled_200.csv',index=0)
